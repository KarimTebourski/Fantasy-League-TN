...
class _PlayersCreationDetailsViewState extends State<PlayersCreationDetailsView> {
  ...
  void initState() {
    ...
    if (widget.playerIndex < 2) {
      _players = List<Player>.from(players.where((player) => player.position=="Goalkeeper"));
    } else if (widget.playerIndex < 7) {
      _players = List<Player>.from(players.where((player) => player.position=="Defender"));
    } else if (widget.playerIndex < 12) {
      _players = List<Player>.from(players.where((player) => player.position=="Midfielder"));
    } else {
      _players = List<Player>.from(players.where((player) => player.position=="Attacker"));
    }
